...
AFRAME.registerComponent('desktop-and-mobile-controls', {
  ...
  onClick: function (evt) {
    if (DeviceManager.isMobile) return; // Mobile interactions via buttons
    if (evt.target.classList.contains('arrow-btn') || evt.target.classList.contains('action-btn') || evt.target.closest('.arrow-controls')) return; // Ignore clicks on controls
    ...
    // Handle clickable elements for navigation
    if (clickedEl.classList.contains('clickable')) {
      console.log("Desktop click on .clickable:", clickedEl.id);
      clickedEl.emit('click', null, false); // Emit standard click
      return; // Don't try to pick up clickable items
    }
  ...
  onTouchEnd: function (evt) {
    // Allow button interactions regardless of swipe
    if (evt.target.classList.contains('arrow-btn') || evt.target.classList.contains('action-btn')) {
      return; // Already handled by button listeners
    }

    // If it was a swipe, don't process as a tap
    if (this.isSwiping) {
        this.isSwiping = false;
        return;
    }

    // Handle tap for pickup/drop (only if not inspecting)
    if (!this.inspectionMode) {
      const currentTime = Date.now();
      const tapLength = currentTime - this.lastTapTime;
      
      // Basic double-tap detection (within 500ms)
      if (tapLength < 500 && tapLength > 0) {
          ...
          if (intersection) {
              ...
              if (tappedEl.classList.contains('clickable')) {
                  console.log("Mobile tap on .clickable:", tappedEl.id);
                  tappedEl.emit('click', null, false); // Emit standard click
                  return; // Don't try to pick up clickable items
              }
              // Handle pickupable items (only if nothing is held)
              if (tappedEl.classList.contains('pickupable') && !this.heldObject) {
                  console.log("Mobile tap: Picking up object", tappedEl.id);
...
AFRAME.registerComponent('arrow-controls', {
  init: function() {
    // Check if we are in VR mode before adding controls
    if (DeviceManager && DeviceManager.isVR) {
      return; // Don't initialize UI in VR mode
    }
